...
def run_download_job(job_id, items, output_dir):
    """Background worker for downloading items."""
    job = jobs[job_id]
    job['status'] = 'running'
    
    downloader = get_downloader()
    # Override output dir if needed, or use the one from config/request
    # For now, we'll use the one passed in or default
    ...
    try:
        for i, item in enumerate(items, 1):
            ...
            try:
                if item_type == 'course':
                    success = downloader.download_course_by_uid(uid, 'pdf', output_dir)
                elif item_type == 'module':
                    success = downloader.download_module(uid, 'pdf', output_dir)
                else:
                    success = downloader.download_learning_path(learning_path_uid=uid, output_format='pdf', output_dir=output_dir)
...
def download():
    ...
    thread = threading.Thread(target=run_download_job, args=(job_id, items, output_dir))
    thread.start()
